...
void Webserv::startServer() {
  ...
  and to the global socket_list (to reset the readfds fd_set in startListen()
  and to close everything at the end) Bind() gives a "name" to each socket
  */
  for (std::map<std::string, Server *>::iterator server_it =
           _server_list.begin();
       server_it != _server_list.end(); server_it++) {
    ...
    for (std::vector<int>::iterator port_it = port_list.begin();
      ...
      fcntl(listen_socket, F_SETFL,
            O_NONBLOCK); // Sets the sockets to non-blocking
      _listen_socket_list.push_back(listen_socket);
      server_it->second->addSocket(listen_socket);

      _socketAddr.sin_port =
          htons(*port_it); // Link the socket to the corresponding port
      if (bind(listen_socket, (sockaddr *)&_socketAddr, _socketAddrLen) < 0)
        throw bindException();

      if (listen(listen_socket, MAX_LISTEN) <
          0) // The second argument is the max number of connections the socket
             // can take at a time
        throw listenException();
    }
  }
  if (!defaultPortIsSet(
          full_port_list)) // I chose to use a default port to access things
                           // like files, stylesheets and favicons...
  {
    listen_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (listen_socket < 0)
      throw openSocketException();
...
void Webserv::startListen() {
  ...
  */
  ...
  while (!kill) {
    ...
    readfds with the listening sockets Go through the whole process again
    */
